-- ============================================================================
-- ENABLE PRE-REGISTRATION FOR DRIVERS
-- ============================================================================
-- Objectif : Permettre à l'admin de créer un dossier chauffeur (véhicule, etc.)
-- SANS créer de compte utilisateur. Le chauffeur créera son compte lui-même via l'app.
-- ============================================================================

BEGIN;

-- 1. Rendre user_id optionnel dans la table drivers
ALTER TABLE public.drivers ALTER COLUMN user_id DROP NOT NULL;

-- 2. Mettre à jour le Trigger d'inscription (handle_new_user)
-- Pour qu'il "récupère" le dossier chauffeur existant (basé sur l'email) au lieu d'échouer ou de créer un doublon.
...
BEGIN
    ...
    IF u_role = 'chauffeur' OR u_role = 'driver' THEN
        ...
        ELSE
            ...
            INSERT INTO public.drivers (user_id, first_name, last_name, email, phone, status)
            VALUES (NEW.id, u_fname, u_lname, NEW.email, u_phone, 'offline')
...
-- 3. Mettre à jour la fonction RPC create_driver_user
-- Elle doit maintenant accepter des username/password NULL
...
BEGIN
    ...
    ELSE
        ...
        -- Mais pour la pré-inscription, on a besoin d'un email pour le matching futur
        -- On va supposer que le frontend envoie l'email dans le champ 'username' ou on devrait ajouter un champ email distinct
        -- Pour l'instant, on utilise le username comme email de contact
        v_email := username; 
        -- ATTENTION : Si username est vide, on a un problème. 
        -- On va supposer que le frontend envoie l'email dans 'username' même si c'est pas un login.
    END IF;
