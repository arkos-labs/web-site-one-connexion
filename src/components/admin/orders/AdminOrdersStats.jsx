    ...
    return (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 animate-in fade-in slide-in-from-bottom-4 duration-700 delay-100">
            {[
                ...
                { label: "À Assigner", count: activeOrders.filter(o => o.status === 'accepted').length, color: "text-[#ed5518]", bg: "bg-[#ed5518]", icon: "🧭" },
                { label: "En mission", count: activeOrders.filter(o => ['assigned', 'driver_accepted', 'in_progress', 'picked_up'].includes(o.status)).length, color: "text-amber-600", bg: "bg-amber-50", icon: "🚚" },
                { label: "Total Livrées", count: historyOrders.filter(o => o.status === 'delivered').length, color: "text-[#ed5518]", bg: "bg-[#ed5518]", icon: "💰" },
            ].map((stat, i) => (
