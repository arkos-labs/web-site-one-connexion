...
function statusColor(status) {
  switch (status) {
    ...
    case "accepted": return "bg-[#ed5518] text-[#ed5518]";
    case "assigned": return "bg-amber-50 text-amber-600";
    case "driver_accepted": return "bg-[#ed5518] text-[#ed5518]";
    case "in_progress": return "bg-[#ed5518] text-[#ed5518]";
    ...
    <div className="min-h-screen bg-[#f8fafc] md:p-4">
      <div className="mx-auto max-w-7xl space-y-10">
        ...
        <header className="relative overflow-hidden rounded-[2.5rem] bg-slate-900 p-10 text-white shadow-2xl md:p-14">
          <div className="absolute -right-20 -top-20 h-64 w-64 rounded-full bg-[#ed5518] opacity-20 blur-3xl mix-blend-screen animate-pulse"></div>
          <div className="absolute -bottom-32 -left-20 h-80 w-80 rounded-full bg-[#ed5518] opacity-20 blur-3xl mix-blend-screen animate-pulse" style={{ animationDelay: '2s' }}></div>
          ...
          <div className="relative z-10 flex flex-col gap-8 md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex shrink-0 items-center gap-4">
              <button className="group relative flex h-14 w-14 items-center justify-center rounded-2xl bg-white/10 text-white shadow-sm ring-1 ring-white/20 backdrop-blur-md transition-all duration-300 hover:scale-105 hover:bg-white hover:text-slate-900 focus:outline-none focus:ring-4 focus:ring-[#ed5518]/30">
                <Bell size={24} className="transition-transform group-hover:rotate-12" strokeWidth={2} />
                {stats.activeCount > 0 && (
                  <span className="absolute -right-1 -top-1 flex h-4 w-4 items-center justify-center rounded-full bg-[#ed5518] text-[9px] font-bold ring-2 ring-slate-900">
                    {stats.activeCount}
                  </span>
                )}
              </button>
              <button
                ...
                className="group flex h-14 items-center gap-3 rounded-2xl bg-gradient-to-r from-primary to-primary px-6 font-bold text-white shadow-xl shadow-primary/20 transition-all duration-300 hover:scale-105 hover:shadow-primary/40"
              >
                Commander
        ...
        {loading ? null : (
          <>
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              ...
              <div className="group relative overflow-hidden rounded-[2.5rem] bg-white p-8 shadow-sm ring-1 ring-slate-100 transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl hover:shadow-primary/10 hover:ring-[#ed5518]">
                ...
                <div className="relative z-10 flex items-start justify-between">
                  ...
                  <div className="rounded-3xl bg-[#ed5518]/50 p-4 text-[#ed5518] shadow-inner transition-transform duration-500 group-hover:rotate-12 group-hover:scale-110">
              ...
              <div className="group relative overflow-hidden rounded-[2.5rem] bg-white p-8 shadow-sm ring-1 ring-slate-100 transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl hover:shadow-blue-500/10 hover:ring-blue-100">
                <div className="absolute -right-10 -top-10 h-32 w-32 rounded-full bg-[#ed5518] transition-transform duration-500 group-hover:scale-150"></div>
                <div className="relative z-10 flex items-start justify-between">
                  <div>
                    ...
                    <div className="mt-4 flex items-end gap-3">
                      ...
                      <span className="mb-1 text-sm font-bold text-[#ed5518]">réussies</span>
                    </div>
                  </div>
                  <div className="rounded-3xl bg-[#ed5518]/50 p-4 text-[#ed5518] shadow-inner transition-transform duration-500 group-hover:-rotate-12 group-hover:scale-110">
            ...
            <div className="grid gap-8 lg:grid-cols-[1.5fr_1fr]">
              ...
              <div className="flex flex-col gap-6">
                <div className="flex items-center justify-between px-2">
                  ...
                  <button onClick={() => navigate('/dashboard-client/orders')} className="group flex items-center gap-2 text-sm font-bold text-[#ed5518] transition-colors hover:text-[#ed5518]">
                    Gérer
                ...
                <div className="flex flex-col gap-4">
                  {recentOrders.length === 0 ? (
                    <div className="flex flex-col items-center justify-center rounded-[2.5rem] bg-white py-16 text-center shadow-sm border border-slate-100 min-h-[300px]">
                      ...
                      <button onClick={() => navigate('/dashboard-client/nouvelle-course')} className="mt-6 rounded-2xl bg-slate-900 px-6 py-3 font-bold text-white transition-all hover:bg-[#ed5518] hover:shadow-lg hover:shadow-primary/20">
                        Créer une course
                      </button>
                    </div>
                  ) : (
                    recentOrders.map((d, i) => (
                      ...
                      >
                        <div className="absolute left-0 top-0 h-full w-1.5 bg-gradient-to-b from-primary to-primary opacity-0 transition-opacity duration-300 group-hover:opacity-100" />
                      ...
                      {invoices.map((inv) => (
                        <div key={inv.id} className="group flex items-center justify-between p-4 transition-colors hover:bg-slate-50 rounded-2xl">
                          <div className="flex items-center gap-4">
                            <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-[#ed5518] text-[#ed5518]">
                              <FileText size={20} />
                            </div>
                            <div>
...
function NavItem({ icon: IconComponent, label, active, badge, to = "#" }) {
  ...
  return (
    <a href={to} className={classes}>
      ...
      {badge && (
        <span className="flex h-5 w-5 items-center justify-center rounded-full bg-[#ed5518] text-[10px] text-white">
          {badge}
        </span>
      )}
    </a>
