...
const benefits = [
  ...
  { title: "Facturation Logistique", desc: <span>Mensuelle, détaillée et conforme <a href="https://fnfe-mpe.org/factur-x/" target="_blank" rel="noopener noreferrer" className="underline decoration-primary/30 hover:decoration-primary">Factur-X</a> pour simplifier votre comptabilité.</span> },
...
const faqs = [
  { q: "Comment fonctionne le bon de commande ?", a: <span>C'est simple et automatique : dès que votre course est validée par notre système, un Bon de Commande (BC) officiel est généré. Il est instantanément accessible dans votre <Link to="/connexion" className="text-[#ed5518] font-medium hover:underline">espace client</Link> et envoyé par email. Ce document comprend tous les détails logistiques et fait foi de l'ordre de mission.</span> },
  ...
  { q: "Puis-je ouvrir un compte pro sans engagement ?", a: <span>Oui, la création d'un <Link to="/inscription" className="text-[#ed5518] font-medium hover:underline">compte professionnel</Link> est 100% gratuite et sans aucun engagement de durée. Vous ne payez que ce que vous consommez. Pas d'abonnement mensuel caché, pas de frais de dossier.</span> },
  ...
  return (
    <div className="bg-slate-50 text-slate-700 font-sans selection:bg-[#ed5518]/30 selection:text-[#ed5518]">
      ...
      <section id="features" className="bg-white py-16 md:py-32 relative border-t border-slate-100">
        ...
        <div className="mx-auto max-w-6xl px-4 md:px-8 relative z-10">
          ...
          <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
            {benefits.map((b, i) => (
              <div key={i} className="group relative rounded-3xl bg-white p-8 shadow-xl shadow-slate-200/50 border border-slate-100 hover:bg-slate-50 transition-all hover:-translate-y-2 hover:border-[#ed5518]/30">
                <div className="mb-6 inline-flex h-14 w-14 items-center justify-center rounded-2xl bg-orange-50 text-[#ed5518] group-hover:bg-[#ed5518] group-hover:text-white transition-all transform group-hover:scale-110">
          ...
          <div className="mt-20 text-center">
            <Link to="/commande-sans-compte" className="inline-flex items-center gap-2 rounded-full bg-slate-900 px-8 py-4 text-sm font-bold text-white uppercase tracking-widest shadow-xl shadow-slate-900/10 transition-transform hover:scale-105 hover:bg-[#ed5518]">
      ...
      <section id="expertises" className="bg-slate-50 py-16 md:py-32 relative border-t border-slate-100">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-[#ed5518]/10 blur-[150px] rounded-full point-events-none"></div>
        <div className="mx-auto max-w-6xl px-4 md:px-8 relative z-10">
          <div className="mb-20 text-center">
          ...
          <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
            {expertises.map((e, i) => (
              <div key={i} className="group flex flex-col items-center rounded-3xl bg-white p-10 text-center shadow-xl shadow-slate-200/40 border border-slate-100 transition-all hover:-translate-y-2 hover:bg-white hover:border-[#ed5518]/20">
                <div className="mb-8 rounded-full bg-orange-50 p-5 text-[#ed5518] group-hover:bg-[#ed5518] group-hover:text-white transition-all transform group-hover:rotate-12">
                ...
                <Link to={e.link} className="mt-auto px-6 py-2.5 text-xs font-bold uppercase tracking-widest text-[#ed5518] hover:text-white hover:bg-[#ed5518] rounded-full border border-[#ed5518]/30 transition-all">
                  Découvrir
                </Link>
              </div>
            ))}
          ...
          <div className="grid gap-8 md:grid-cols-3">
            {testimonials.map((t, i) => (
              <div key={i} className="flex flex-col rounded-3xl border border-slate-200 bg-white/60 backdrop-blur-sm p-10 shadow-xl shadow-slate-200/50 transition-all hover:border-slate-300 hover:bg-white hover:-translate-y-2">
                <div className="mb-8 flex gap-1.5 text-[#ed5518]">
                  {[...Array(5)].map((_, i) => (
                ...
                <div className="flex items-center gap-4 pt-6 border-t border-slate-100">
                  <div className={`flex h-12 w-12 items-center justify-center rounded-xl text-base font-black text-white shadow-lg ${i === 0 ? 'bg-indigo-500' : i === 1 ? 'bg-[#ed5518]' : 'bg-emerald-500'}`}>
                  ...
                  <div>
                    ...
                    <div className="text-[10px] font-bold uppercase tracking-widest text-slate-500 mt-0.5 flex items-center gap-1.5">
                      <CheckCircle2 size={12} className="text-[#ed5518]" /> Client Certifié
                    </div>
                  </div>
                </div>
              </div>
            ...
            {faqs.map((f, i) => (
              <details key={i} className="group rounded-3xl bg-slate-50 p-2 shadow-sm border border-slate-200 [&_summary::-webkit-details-marker]:hidden transition-all hover:border-slate-300">
                <summary className="flex cursor-pointer items-center justify-between rounded-2xl px-8 py-6 font-bold tracking-tight text-slate-900 transition-colors hover:bg-slate-100">
                  ...
                  <div className="grid h-10 w-10 flex-shrink-0 place-items-center rounded-full bg-white border border-slate-200 text-slate-400 transition-all group-open:rotate-45 group-open:bg-[#ed5518] group-open:border-[#ed5518] group-open:text-white group-hover:bg-orange-50 group-hover:text-[#ed5518] group-open:group-hover:text-white">
                    <Plus size={20} />
                  </div>
                </summary>
