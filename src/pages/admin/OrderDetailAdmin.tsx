...
export default function OrderDetailAdmin() {
    ...
    const fetchAvailableDrivers = async () => {
        try {
            ...
            const availableDrivers = allDrivers.filter(
                (d) => d.status === "available" && d.is_online
            );
    ...
    const handleDispatchOrder = async () => {
        ...
        try {
            ...
            toast.success("Commande dispatchée");
        ...
        <div className="container mx-auto p-6 max-w-7xl space-y-6">
            ...
            <Dialog open={showDispatchDialog} onOpenChange={setShowDispatchDialog}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>
                            {order.driver_id ? "Changer de chauffeur" : "Dispatcher la commande"}
                        </DialogTitle>
                        <DialogDescription>
                                ...
                                {drivers.length > 0 ? (
                                    drivers.map((driver) => (
                                        <SelectItem key={driver.id} value={driver.id}>
                                            {driver.first_name} {driver.last_name} - {driver.vehicle_type || "Véhicule non renseigné"}
                                        </SelectItem>
                                    ))
                                ) : (
                    ...
                    <DialogFooter>
                        ...
                        <Button onClick={handleDispatchOrder} disabled={!selectedDriver || actionLoading}>
